...
class User:
    ...
    def to_dict(self):
        '''
        Utillity function to convert user information to dict. 
        '''
        return {
            "user_id": self.user_id,
            "password": self.password,
            "is_logged_in": self.is_logged_in,
            "queued_messages": [ChatService.message_to_dict(msg) for msg in self.queued_messages]
        }
...
class ChatService(chat_pb2_grpc.ChatServiceServicer):
        ...
        try:
            with open('users.json', 'r') as f:
                ...
                for user_data in users_data:
                    ...
                    user.queued_messages = [ChatService.dict_to_Message(
                        msg) for msg in user_data['queued_messages']]
    ...
    @staticmethod
    def message_to_dict(msg: chat_pb2.Message):
        '''
        Utility function to convert message to dict.
        '''
        return {
    ...
    def dict_to_Message(msg: dict):
        '''
        Utility function to convert dict to message.
        '''
    ...
    def SignUp(self, request, context):
        '''
        Sign up function that checks whether username already exists. 
        '''
    ...
    def ListUsers(self, request, context):
        '''
        List all signed up users with the option of wildcard.
        '''
        if request.query == "*":
            for user in self.users.values():
    ...
    def SendMessage(self, request, context):
        '''
        Send Message function that checks the validity of recipient and checks whether the recipient is logged in;
        if not log in, the program will queue the message.
        '''
    ...
    def ReceiveMessage(self, request, context):
        '''
        Receive Function that prints message to the screen.
        '''
    ...
    def DeleteAccount(self, request, context):
        '''
        Delete account function that supports authentication. 
        '''
        if (request.user_id not in self.users) or (self.users[request.user_id].password != request.password):
            context.set_code(grpc.StatusCode.UNAUTHENTICATED)
            context.set_details('Authentication failed')
